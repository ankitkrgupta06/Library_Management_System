...
import { Blocks, Eye, EyeOff, LockKeyhole, ShieldCheck, UserRound ,Mail, ArrowRight} from 'lucide-react'
...
function Login() {
  ...
  const handleSubmit=async(event)=>{
    ...
    try {
      ...
      setLoading(false);
      const fallbackPath=form.role==="admin"?"/admin/dashboard":"/user/dashboard";
      let target=location.state?.from||fallbackPath;

      if(form.role==="user" && typeof target==="string" && target.startsWith("/admin")){
        console.warn("Login:preventing navigation to admin route for student;using fallback");
        target=fallbackPath;
      }else if(form.role==="user" && typeof target==="string" && target.startsWith("/user")){
        console.warn("Login:preventing navigation to admin route for student;using fallback");
        target=fallbackPath;
      }
      ...
      setError("An unexpexted connection error occurred:"); 
    }
  }
  return (
        ...
        <section className={s.infoPanel}>
          ...
          <div className={s.infoBoxesContainer}>
            ...
            <div className={s.infoBox}>
              ...
              <p className={s.infoBoxText}>
                Log in using yout registered admin account to access the
            ...
            <form className={s.form} onSubmit={handleSubmit}>
              <div className={s.roleContainer}>
                ...
                <div className={s.roleGrid}>
                  {roleChoices.map((choice)=>{
                    ...
                    return(
                      <label key={choice.value} className={`${s.roleOption}${form.role===choice.value?s.roleOptionSelected:s.roleOptionUnselected}`}>
                        <input type="radio" name="role" value={choice.value} checked={form.role===choice.value} onChange={handleChange} className={s.roleRadio}/>
              ...
              <div className={s.footerFlex}>
                <span className={s.footerText}>
                  {form.role==="admin"?"Admin accounts use existing credentials":"Student signup is available below"}
                </span>
                {form.role==="user" &&(
                  <Link to='/signup' className={s.signupLink}>Create Account</Link>
                )}
              </div>
