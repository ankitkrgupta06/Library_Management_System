...
function AdminBookPage() {
    ...
    searchTimeoutRef.current=window.setTimeout(async()=>{
      try {
        ...
        const response=await fetch(
          `http://localhost:5000/api/students/search-by-roll?roll=${encodeURIComponent(issueForm.rollNumber.trim(),)}`,
          {
            headers:{
        ...
        console.error("Unable to fetch matching students");
      }finally{
  ...
  const handleBookChange=(bookId,field,value)=>{
    ...
    setIssueForm((current)=>({
      ...current,
      books:current.books.map((book)=>book.id===bookId?{...book,[field]:value}:book)
    }))
  }
  ...
  const removeBookDraft=(bookId)=>{
    setIssueForm((current)=>({
      ...current,
      books:current.books.length>1?current.books.filter((book)=>book.id!==bookId):current.books,
    }))
  }
      ...
      <div className={s.mainSection}>
        <div className={s.innerContainer}>
          ...
          <form className={s.form} onSubmit={handleIssueSubmit}>
            <div className={s.formGrid}>
              <label className={s.label}>
                ...
                <div className={s.searchInputWrapper}>
                  ...
                  <input type="text" name="studentName" value={issueForm.studentName} readOnly placeholder='Selected student name' className={s.readonlyInput}/>
                </div>
              </label>

              <label className={s.label}>
                <span className={s.labelSpan}>Department</span>
                <input type="text" name="department" value={issueForm.department} readOnly={isStudentSelected} onChange={handleIssueChange} placeholder='Department' className={s.textInput}/>
              </label>

              <label className={s.label}>
                <span className={s.labelSpan}>Stream</span>
                <input type="text" name="stream" value={issueForm.stream} readOnly={isStudentSelected} onChange={handleIssueChange} placeholder='Stream' className={s.textInput}/>
              </label>

              <label className={s.label}>
                <span className={s.labelSpan}>Year</span>
                 <input type="text" name="academicYear" value={issueForm.academicYear} readOnly={isStudentSelected} onChange={handleIssueChange} placeholder='Year' className={s.textInput}/>
              </label>

              <label className={s.label}>
                <span className={s.labelSpan}>Semester</span>
                <input type="text" name="semester" value={issueForm.semester} readOnly={isStudentSelected} onChange={handleIssueChange} placeholder='Semester' className={s.textInput}/>
              </label>

              <label className={s.label}>
                <span className={s.labelSpan}>Roll Number</span>
                <input type="text" name="rollNumber" value={issueForm.rollNumber} readOnly={isStudentSelected} onChange={handleIssueChange} placeholder='Search by roll number' className={s.textInput}/>
              </label>
            </div>
            ...
            {formMessage && <div className={s.formMessage}></div>}
