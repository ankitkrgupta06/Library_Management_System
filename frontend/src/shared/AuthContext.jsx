    ...
    try {
      ...
      if(response.ok){
        ...
        if(data.success && Array.isArray(data.users)){
          const fetchedAccounts=data.users.map(mapUserToFrontend)
    ...
    const initializeAuth=async()=>{
      ...
      if(token && session){
        try {
          ...
          console.error("Backend init auth failed,,falling back to local session",error);
          try {
...
[]);
    ...
    try {
      ...
      if(data.success && data.token && data.user){
        ...
        if(role && mappedUser.role!==role){
          console.warn("AuthContext:Role mismatch ,Expected:",role,"Got:",mappedUser.role);
          return{
            ok:false,
            error:role==="admin"?"This account is not an admin account":"This account is not a student account"
          }
        }
      ...
      return{
        ok:false,
        error:"Server connection failed, Please ensure backend is running on http://localhost:5000"
      };
    }
  }
  ...
  const completeProfileData=async({email,department,stream,semester,academicYear,rollNumber})=>{
    try {
