      ...
      <aside className={`${s.sidebar}${open?s.sidebarOpen:s.sidebarClosed}`}>
        <div className={`${s.sidebarHeader}`}>
          <div className="min-w-0 pr-3">
            ...
            {badge && <span className={`${s.badgeBase}${badgeStyles}`}>{badge}</span>}
          </div>
            ...
            return(
              <Link key={item.label} to={item.href} onClick={()=>setOpen(false)} className={`${s.navLink}${active? s.navLinkActive:s.navLinkInactive}`}>
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className={`${s.navDescription}${active?s.navDescriptionActive:s.navDescriptionInactive}`}>{item.description}</span>
                </span>
        ...
        <div className={s.footer}>
          {footerItems.map((item)=>{
            ...
            if(item.action){
              return(
                <button key={item.label} type='button' onClick={item.action} className={`${s.footerButton}${item.kind==='primary'?s.footerButtonPrimary:s.footerButtonSecondary}`}>
            ...
            return(
              <Link key={item.label} to={item.href} className={`${s.footerLink}${item.kind==='primary'?s.footerLinkPrimary:s.footerLinkSecondary}`}>
                <span className={s.footerIconWrapper}>
                  <Icon size={16}/>{item.label}
                </span>
                <ChevronRight size={16}/>
